...
def run_sgd(n_bins, max_rules, min_support, alpha_gain, iteration, model, dataset, bin_split="qcut", include_probability = False):
    train_df, test_df, train_ycls, test_ycls = read_data(iteration, model, dataset, n_bins, bin_split=bin_split, include_probability= include_probability)
    print("In run_sgd")
    ...
    print(model)
    fn = f"RQ 3/results/{res_key}/SGRules_{model_name}_{dataset}_{iteration}_{alpha_gain}_bins{n_bins}.txt"
    with open(fn, 'w') as f:
        f.write(str(model))
    print(dir(model))
...
def assign_subgroups(rsd, X, df_fn_toappend, y_unc, y_cls, train_test, model_name, dataset, iteration, n_bins, max_rules, min_support, alpha_gain):
    ...
    to_output.to_csv(f"RQ 3/input/{res_key}/subgroup_assignments_{model_name}_{dataset}_{train_test}_{iteration}_{max_rules}_{min_support}_{alpha_gain}_bins{n_bins}.csv")
    return to_output
...
if __name__ == "__main__":
    ...
    for iteration, n_bins, max_rules, min_support, alpha_gain in combos:
        ...
        print(settings)
        # try:
        res, rsd = run_sgd(n_bins, max_rules, min_support, alpha_gain, iteration, model, dataset, bin_split=bs, include_probability=include_prob)
    ...
    print(all_res_df)
    try:
        os.system("mkdir RQ 3/results/{0}".format(res_key))
    except: pass
    all_res_df.to_csv("RQ 3/results/{2}/subgroup-discovery_{0}_{1}_categorical.csv".format(model, dataset, res_key)) # change to formatted name